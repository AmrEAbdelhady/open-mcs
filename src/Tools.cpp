...
/*! \brief execute an clique algorithm, time it,
           and print the size of the maximum clique
           along with time information.
    \param pAlgorithm a pointer to a maximum clique algorithm object
    \param cliques a list of lists, storing the largest clique found so far
    \param outputLatex flag to indicate if latex formatting should be used for output
*/
...
void RunAndPrintStats(Algorithm *pAlgorithm, list<list<int>> &cliques, bool const outputLatex)
{
    ...
    int const cliqueNumber(cliques.empty()? -1: cliques.back().size());

    if (!outputLatex) {
        fprintf(stderr, "%d,", cliqueNumber);
