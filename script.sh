...
mkdir "$output_dir"
rm -rf $old_output_dir/log.*
rm -rf $old_output_dir/$split_file_prefix*
rm -rf $csv_dir
mkdir "$csv_dir"
...
cd $output_dir
ls -1 $full_data_dir/ | grep ".*\.graph$" | split --lines=$num_lines_per_core - $split_file_prefix
cd $current_dir
...
function process_split_file () {
    ...
    for file_name in `cat $split_file_name`; do
        ...
        bin/open-mcs --input-file=file_name [--compute-independent-set] --algorithm=mcs >> $log_file
